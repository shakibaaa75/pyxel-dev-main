...
export default function BlogSection() {
  ...
  if (loading) {
    return (
      <div className="bg-[#161616] flex items-center justify-center py-24">
      ...
      >
        ...
        >
          {posts.map((post) => (
            <div
              key={post.id}
              onMouseEnter={() => setHovered(Number(post.id))}
              onMouseLeave={() => setHovered(null)}
              className="flex-none w-[calc(33.333%-20px)] min-w-[280px] md:min-w-[300px]"
              style={{ scrollSnapAlign: "start" }}
            >
              ...
              <h3
                className="text-[17px] font-bold mb-2.5 leading-[1.4] transition-colors duration-200"
                style={{
                  color: hovered === Number(post.id) ? "#2979FF" : "#fff",
                }}
              >
                {post.title}
              </h3>
